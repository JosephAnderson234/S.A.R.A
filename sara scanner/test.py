...
class ScannerQR(tk.Toplevel):
    ...
    async def capture(self):
        self.capture_task = asyncio.current_task()
    ...
    def capta(self, frm):
        ...
        cv2.putText(frm, "Muestre el código", (84, 37), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
        cv2.putText(frm, "frente a la cámara", (80, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
            ...
            for code in self.info:
                if code not in self.codelist:
                    ...
                    if content.isnumeric():
                        try:
                            cursor = conexion.cursor()
                            cursor.execute('SELECT * FROM students WHERE code_student = {0}'.format(content))
